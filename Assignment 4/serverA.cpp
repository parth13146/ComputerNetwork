...
int main(void){
    cout << "Server A is up and running using UDP on port "<<serverAPort<< endl;
    ...
    int rv = getaddrinfo(LOCALHOST, serverAPort, &hints, &servinfo);
    if (rv != 0) {
    ...
    while(1){
        ...
        cout << "Server A has received a request for finding possible friends of User " <<queriedUserID << " in "<< queriedState<<endl;
        ...
        if(temp2.compare("NA")==0){
            ...
            cout << "Server A has sent User " << queriedUserID<<" not found to Main Server" << endl;
        }else{
            cout << "Server A found the following possible friends for User " << queriedUserID <<" in " << queriedState <<" : "<<temp2<<endl;
            memset(message, 0, MAXBUFLEN);
            int z=0;
            for(z=0;z<temp2.length();z++){
                message[z] = temp2[z];
